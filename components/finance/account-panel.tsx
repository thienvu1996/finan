...
type AccountState = { authenticated: boolean; user?: { email?: string }; role?: "admin" | "member"; onboardingCompleted?: boolean; subscription?: { plan_id: string; status: string; ends_at: string | null }; connection?: { connected: boolean; mode?: string; bankCount?: number }; bankAccounts?: ConnectedBankAccount[]; billingConfigured?: boolean };
export default function AccountPanel({ plans, plansVisible, open, onOpenChange, onIdentity, onConnected, onSignedOut }: { plans: Plan[]; plansVisible: boolean; open: boolean; onOpenChange: (v: boolean) => void; onIdentity: (v: { email: string; plan: string } | null) => void; onConnected: () => void; onSignedOut: () => void }) {
  ...
  async function saveBalance(event: React.FormEvent<HTMLFormElement>, accountId: string) {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const raw = String(formData.get("balance") || "").trim();
    const balance = Number(raw);
    if (!Number.isSafeInteger(balance) || balance < 0) return setError("Số dư phải là số nguyên VND từ 0 trở lên.");
    ...
    const body = await response.json(); setBusy(false);
    if (!response.ok) return setError(body.message || "Chưa thể lưu số dư.");

    await refresh();
    onConnected();
    setMessage("Đã lưu số dư hiện tại. Giao dịch realtime mới sẽ tự cộng/trừ từ mốc này.");
  }
