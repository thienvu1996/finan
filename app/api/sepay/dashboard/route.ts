...
export async function GET(request: Request) {
  try {
    ...
    const [accounts, flow] = await Promise.all([getBankAccounts(token, connection.mode as SePayMode), getTransactions(token, connection.mode as SePayMode, month)]);
    ...
    const allowedAccounts = accounts.slice(0, plan?.max_bank_accounts ?? 1);
